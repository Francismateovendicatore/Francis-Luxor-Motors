  ...
  if (!authed) return (
    <section className="adm-gate">
      <div className="adm-gate__inner">
        <div className="adm-gate__sigil">✦</div>
        ...
        <form className="adm-gate__form" onSubmit={handleLogin}>
          <div className={`adm-gate__field ${pwErr ? "adm-gate__field--error" : ""}`}>
            <input
              ...
              placeholder="Enter access code" className="adm-gate__input" autoComplete="current-password"
            />
          </div>
          {pwErr && <p className="adm-gate__err">Invalid credentials. Access denied.</p>}
          <button type="submit" className="adm-gate__btn">AUTHENTICATE</button>
        </form>
      </div>
    </section>
  );
    ...
    <section className="adm">
      ...
      <div className="adm__header">
        ...
        <div className="adm__tabs">
          <button className={`adm__tab ${tab==="add" ? "adm__tab--active" : ""}`} onClick={() => { setTab("add"); setEdit(null); setForm(EMPTY); }}>
            {editSlug ? "✎ Edit Vehicle" : "+ Add Vehicle"}
          </button>
          <button className={`adm__tab ${tab==="manage" ? "adm__tab--active" : ""}`} onClick={() => setTab("manage")}>
      ...
      {tab === "add" && (
        <form className="adm__form" onSubmit={handleSubmit}>
          ...
          <div className="adm__grid adm__grid--3">
            ...
            <Field label="Accent Color (hex)" value={form.accent} onChange={v => set("accent", v)} placeholder="#D4AF37" extra={
              <span className="adm__color-dot" style={{ background: form.accent }} />
            } />
          </div>
          ...
          <div className="adm__grid adm__grid--1">
            ...
            <Field label="Engine Description" type="textarea" value={form.engine_desc} onChange={v => set("engine_desc", v)}
              placeholder="8.0-litre quad-turbocharged W16 engine producing..." rows={3} />
          </div>

          <div className="adm__actions">
            {editSlug && (
              <button type="button" className="adm__btn adm__btn--ghost" onClick={() => { setEdit(null); setForm(EMPTY); }}>
                Cancel Edit
              </button>
            )}
